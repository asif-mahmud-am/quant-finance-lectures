...
class dailyBars:
    ...
    def rollingPCA(self, features, window=1, num_pc=1):
        ...
        pct_ret_.fillna(0,inplace=True)
        eig_val = list()
        eig_vec = list()
    ...
    def rollingfactor(self):
        ...
        self.fct_rl = pd.DataFrame(np.zeros((self.features.shape[0], self.num_pc)),
                                    index=self.features.index,
...
class minuteBars:
    ...
    def ohlcvData(self, start, end, value_list=['close']):    
        ...
        for value in value_list:
            df_value = self.data.get_history_window(self.assets,
                            end_dt = pd.Timestamp(end, tz='utc'),
                            bar_count = len(self.calendar.minutes_in_range(pd.Timestamp(start),pd.Timestamp(end))),
                            frequency = '1m',
                            field = value,
    ...
    def pctReturn(self, start, end, periods=1):
        ...
        price = self.data.get_history_window(self.assets,
                                        end_dt = pd.Timestamp(end, tz='utc'),
                                        bar_count = len(self.calendar.minutes_in_range(pd.Timestamp(start),pd.Timestamp(end)))+periods,
                                        frequency = '1m',
                                        field = 'close',
                                        data_frequency = 'minute'
                                        )
    ...
    def rollingPCA(self, features, window=1, num_pc=1):
        ...
        start = self.calendar.minutes_window(pd.Timestamp(start, tz='utc'),-(window-1))[0].strftime(timeFormat)
    ...
    def rollingfactor(self):
        ...
        # Create a df containing row indices for the workaround
        df_idx = pd.DataFrame(np.arange(self.features.shape[0]-1))
